...
const MetadataLoaderContext = createContext<
  MetadataLoaderContextType | undefined
>(undefined);
...
export const MetadataLoaderProvider: React.FC<MetadataLoaderProviderProps> = ({
  children,
}) => {
  const { connection } = useFlecsConnection();
  const [moduleMetadataMap, setModuleMetadataMap] = useState<
...
export const useMetadataLoader = (): MetadataLoaderContextType => {
  ...
  if (context === undefined) {
    throw new Error(
      "useMetadataLoader must be used within a MetadataLoaderProvider",
    );
  }
  return context;
};
