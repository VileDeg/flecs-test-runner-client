...
const convertOperatorsBaseToEntityName = (unitTest: UnitTest): UnitTest => {
  return {
    ...
    operators: unitTest.operators.map((oper) => {
      ...
      const newBase = unitTest.expectedConfiguration.find(
        (e) => e.id === entityId,
      )?.entityName;
      if (!newBase) {
...
export const WorkspaceProvider: React.FC<WorkspaceProviderProps> = ({
  ...
  const displayValidationResults = (results: TestValidationResult[]) => {
    results.forEach((result) =>
      showToast(result.message, result.type ?? MessageType.ERROR),
    );
  };

  const saveToWorkspace = (id: string, testProperties: UnitTestProps) => {
    const wsTest = getTest(id);
  ...
  const runTest = async (
    ...
    if (wsTest.status === TestStatus.RUNNING) {
      showToast(
        `Test "${unitTest.name}" is already running.`,
        MessageType.WARNING,
      );
      return false;
    }
    try {
      ...
      const errMsg = `Test "${unitTest.name}" failed to run: ${errorMessage}`;
  ...
  const validateTest = (
    ...
    if (
      findDuplicates(
        totalTests.map((test) => test.testProperties.test.name),
      ).includes(test.name)
    ) {
