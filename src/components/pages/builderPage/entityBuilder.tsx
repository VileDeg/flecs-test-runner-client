...
export const EntityBuilder: React.FC<EntityBuilderProps> = ({
  ...
  const isEntityHaveComponent = (component: Component): boolean => {
    return configuration.components.some((comp) => comp.id === component.id);
  };
  ...
  const handleOnComponentUpdated = (
    updates: Partial<Component> | null,
      ...
      <div className="space-y-3">
        {entity.components.map((component, componentIndex) => (
          <ComponentBuilder
            ...
            component={component}
            onUpdate={(updates) =>
              handleOnComponentUpdated(updates, componentIndex)
            }
          ></ComponentBuilder>
        ))}
