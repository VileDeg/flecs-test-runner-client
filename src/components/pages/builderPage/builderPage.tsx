...
export const TestBuilder: React.FC<TestBuilderProps> = ({
  ...
  const renderEmptyDisplay = (content: React.ReactNode) => (
  ...
  const renderLoadingDisplay = (text: React.ReactNode) =>
    renderEmptyDisplay(
  ...
  const renderWorldBuilders = () => (
    <div className={"grid grid-cols-2 gap-4"}>
      {renderWorldBuilder(initialConfiguration, (conf) => {
        setInitialConfiguration(conf);
      })}
      {renderWorldBuilder(
        expectedConfiguration,
        (conf) => {
          setExpectedConfiguration(conf);
        },
        true,
      )}
    </div>
  ...
  const renderButtons = () => (
    <div className="flex flex-col flex-wrap gap-4 items-start justify-end">
      <Button variant="destructive" onClick={clearForm} className="gap-2">
    ...
    return renderEmptyDisplay(
      <>
        ...
        <p className="text-muted-foreground mb-6">
          Please select a test from the workspace to edit, or create a new test
          in the workspace first.
        </p>
        <Button variant="outline" onClick={goToWorkspacePage}>
          Go to Workspace
        </Button>
      </>,
    );
