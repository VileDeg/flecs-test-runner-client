...
const TEST_STATUS_STYLE: Record<TestStatus, StatusStyle> = {
  [TestStatus.PASSED]: {
    ...
    color: "bg-green-50 text-green-700 dark:bg-green-950 dark:text-green-400 border-green-200 dark:border-green-800"
  },
  [TestStatus.FAILED]: {
    icon: <XCircle className="h-4 w-4" />,
    color: "bg-red-50 text-red-700 dark:bg-red-950 dark:text-red-400 border-red-200 dark:border-red-800"
  },
  [TestStatus.RUNNING]: {
    icon: <Loader2 className="h-4 w-4 animate-spin" />,
    color: "bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-400 border-blue-200 dark:border-blue-800"
  },
  [TestStatus.INVALID]: {
    icon: <AlertTriangle className="h-4 w-4" />,
    color: "bg-yellow-50 text-yellow-700 dark:bg-yellow-950 dark:text-yellow-400 border-yellow-200 dark:border-yellow-800"
  },
  [TestStatus.TIMEOUT]: {
  ...
  [TestStatus.IDLE]: {
    ...
    color: "bg-gray-50 text-gray-700 dark:bg-gray-950 dark:text-gray-400 border-gray-200 dark:border-gray-800"
  }
} as const;
...
export interface TestListItemProps {
  ...
  isSelected: boolean;
  onToggleSelection: (testId: string, index: number, event: React.MouseEvent) => void;
}
...
const TestListItem: React.FC<TestListItemProps> = ({
  ...
  return (
    <div
      ...
      onClick={(event) => {
        if (isSelectionMode) {
          onToggleSelection(wsTest.id, index, event);
        } else {
